...
exports.UDPDaemonSegmentEmitter = void 0;
...
class UDPDaemonSegmentEmitter {
     ...
     */
    constructor(socket = dgram.createSocket('udp4').unref()) {
        this.socket = socket;
     ...
     */
    async emit(trace) {
        const formatted = trace.map((segment) => '{"format":"json","version":1}' + '\n' + JSON.stringify(segment));
        for (const data of formatted) {
            await new Promise((resolve, reject) => {
                const message = Buffer.from(data);
                this.socket.send(message, 0, message.length, this.daemonConfig.udp_port, this.daemonConfig.udp_ip, (err) => {
                    if (err) {
                        reject(err);
                    }
                    else {
...
exports.UDPDaemonSegmentEmitter = UDPDaemonSegmentEmitter;
//# sourceMappingURL=udp.emitter.js.map